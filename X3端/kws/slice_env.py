#!/usr/bin/env python3
"""slice_env.py — 把长 WAV 按固定秒数切成 N 个片段(纯 Python wave 模块)

用法:
    slice_env.py <输入WAV> <片段秒> <输出目录> [前缀] [--resume]

例:
    slice_env.py /home/root/voice_data/env.wav 2 /home/root/voice_data/negative_env noise

约定: 输出 WAV 16kHz mono(因为 KWS 用)。如果输入不是,会警告但仍按原样切片。

负样本目录通常是 record_wakeword.py 的 negative 输出目录(自己手念的两字词),
这里用独立目录 negative_env 避免命名冲突。训练时把两个目录合并即可。
"""
import wave
import os
import sys
...
def main():
    ...
    with wave.open(args.input, "rb") as wf:
        ...
        print(f"[slice_env] 输入    : {args.input}")
        print(f"[slice_env] 总时长  : {dur:.2f} 秒 ({n_fr} 帧)")
        print(f"[slice_env] 通道    : {n_ch}")
        print(f"[slice_env] 采样率  : {fr} Hz")
        print(f"[slice_env] 位深    : {samp_w * 8} bit")
        print(f"[slice_env] 片段    : {args.slice_sec}s × {slice_frames} 帧")
        print(f"[slice_env] 预计生成: {n_slices} 个片段")
        if remainder > 0:
            print(f"[slice_env] 末尾    : {remainder / fr:.2f}s 不足一片(丢弃)")
        print(f"[slice_env] 输出    : {args.outdir}/{args.prefix}_NNN.wav")
        print()
        ...
        for i in range(n_slices):
            ...
            # 每 20 个报一次进度
            if (i + 1) % 20 == 0:
                print(f"[slice_env] 进度: {i + 1}/{n_slices}")

    print()
    print(f"[slice_env] ✅ 完成")
    print(f"[slice_env] 新生成: {ok}")
    if skip > 0:
        print(f"[slice_env] 跳过  : {skip} (--resume)")
