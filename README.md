...
| STM32 Firmware | ✅ 站立 + UART 命令 + motions 模块;踏步骨架就绪,未实测 |
| High-level MCU | ⏳ 旭日 X3 2.0 待到货 |

### v1 板子当前状态(2026-09-13)

- **8 路舵机标定完成**:全部居中 (Pulse=1500, 90°) → 大腿垂直地面 + 小腿水平向前,4 脚实测承重
- **固件进度**:
  - STM32CubeMX 8 外设配齐 (TIM1/2/3/17 × 8 路 PWM + I2C1 + USART1 + SWD + TIM6)
  - SYSCLK = 168 MHz (HSE 8MHz × PLL ×42 / 2)
  - `motions.c/h`(2026-09-13 新):`MOTION_ID` 宏切动作,默认 STAND;改 `MOTION_TROT` = 10s 后踏步
- **当前 UART 命令**(实测代码,2026-09-11):
  - `<id 0-7> <pulse>` 设单路舵机、`all <pulse>` 设全部 8 路、`center` 全部 1500、`stand` STAND 姿态
  - `step trot` 启动 trot 踏步(TIM6 100Hz)、`step stop` 停止、`step show` 调试(目前空)
- **踏步骨架(2026-09-12 重写)**:
  - 远场近似 1mm ≈ 4µs PWM 调整,sin² 抬腿曲线(sin²(0)=0 → 无启动跳变)
  - 8 路舵机全程在 SERVO_LIMIT 内
- **历史**:
  - `sit` 命令已不实现(用户决策,代码里只剩"8 路 1500"等同 `center`)
